...
image_info(imgs)
  ...
  # 画像の高さ
  # 11_08_image-info-details.R
image_info(imgs)$height

  # 画像の変換
  # 11_09_image-convert.R
...
plot(annotated_25)
  ...
  # 横方向への結合
  # 11_18_image-append-horizontal.R
image_append(imgs[1:24]) |>
  plot() # 横方向

  # 縦方向への結合
  # 11_19_image-append-vertical.R
image_append(imgs[25:27], stack = TRUE) |> # 縦方向
  plot()

  # 間隔を開けた結合
  # 11_20_image-append-border.R
imgs[1:3] |>
...
click_locate_image <- function(img, n = 2){
  ...
  # 画像をクリックした位置の取得
  # 11_31_image-click-locate-image.R
pos <- 
  imgs[25] |>
  image_scale("1200") |>
...
[1] 736 724
  ...
  # 複数画像の同一位置での切り取り
  # 11_32_image-click-locate-image-multi.R
...
click_crop_image <- function(path){
  ...
  # 指定範囲の連続切り取り
  # 11_34_image-click-crop-image.R
...
plot(img_all)
  ...
  # 11_39_image-screenshot-install.R
install.packages("screenshot")
library(screenshot)

  # スクリーンショットのためのアプリのインストール
  # 11_40_image-screenshot-install-screenshot.R
screenshot::install_screenshot()

  # スクリーンショットの保存
  # 11_41_image-screenshot-screenshot.R
...
magick::image_read(ss)
  ...
  # スクリーンショットの保存
  # 11_42_image-screenshot.R
clipboard_img <- save_clipboard_image()
  # shell.exec(clipboard_img)

  # クリップボード画像の自動保存
  # 11_43_image-save-screenshot-code.R
