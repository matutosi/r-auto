...
dplyr::count(answer_mutated, area)  |> print(n = 3)  # 単数回答・単純集計
count_multi(answer_mutated, "apps") |> print(n = 3)  # 複数回答・単純集計

  # クロス集計
  # 02_28_analysis-dplyr-cross-summary.R
dplyr::count(answer_mutated, area, satisfy) |> # 単数回答・クロス集計
  tidyr::pivot_wider(names_from = area, values_from = n, values_fill = 0) |>
  print(n = 3)
answer_mutated |> # 複数回答・クロス集計
  dplyr::group_by(area) |>
